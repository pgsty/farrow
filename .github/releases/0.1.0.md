...
All built-in images are still `testing`, except EOL EL7 which is `deprecated`.
Apple Silicon runs the stock Rocky Linux 8 arm64 image with visible
same-architecture TCG because its 64K-granule kernel is incompatible with HVF.
TCG results are compatibility evidence, not performance evidence.
...
<https://farrow.pgsty.com/docs/start/installation/>.
