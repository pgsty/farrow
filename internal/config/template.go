...
func Template(name, cidr string) ([]byte, error) {
	...
	fmt.Fprintf(&output, "#\n")
	fmt.Fprintf(&output, "# This file is a Pigsty-compatible Ansible inventory. Farrow reads only\n")
	fmt.Fprintf(&output, "# the vm_* variables plus nodename/admin_ip; every other variable is\n")
