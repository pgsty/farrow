...
func (p Probe) projectChecks() []Check {
	...
	if openErr == nil {
		...
		if errors.Is(projectErr, os.ErrNotExist) {
			checks = append(checks, Check{Name: "project-state", Status: Warn, Evidence: "project marker exists but resolved state has not been created", Fix: "run piglet up or piglet validate/plan first"})
		} else if projectErr != nil {
			checks = append(checks, Check{Name: "project-state", Status: Error, Evidence: projectErr.Error(), Fix: "run piglet repair --dry-run; do not hand-edit state"})
		} else {
		...
		if projectErr == nil {
			for _, definition := range projectState.Resolved.Nodes {
				...
				if nodeErr == nil {
					...
					checks = append(checks, Check{Name: checkName, Status: Error, Evidence: nodeErr.Error(), Fix: "run piglet repair --dry-run " + definition.Name})
					continue
				}
				if projectState.Resolved.Network == "user" {
					if transaction, transactionErr := store.ReadTransaction(definition.Name); transactionErr == nil {
						checks = append(checks, Check{Name: "transaction/" + definition.Name, Status: Warn, Evidence: fmt.Sprintf("pending operation %s: %s -> %s", transaction.OperationID, transaction.From, transaction.To), Fix: "run piglet repair --dry-run before another lifecycle operation"})
					} else if !errors.Is(transactionErr, os.ErrNotExist) {
						checks = append(checks, Check{Name: "transaction/" + definition.Name, Status: Error, Evidence: transactionErr.Error(), Fix: "preserve the journal and inspect with piglet repair --dry-run"})
					}
					...
					checks = append(checks, Check{Name: "transaction/" + definition.Name, Status: status, Evidence: evidence, Fix: "run piglet repair --dry-run " + definition.Name})
		...
		checks = append(checks, Check{Name: "project", Status: OK, Evidence: "current directory has no Piglet project marker"})
	}
	...
	if spaceErr != nil {
		...
		if available < 20<<30 {
			status = Warn
			fix = "free space or choose a safe absolute PIGLET_DATA_HOME before creating a project"
		}
