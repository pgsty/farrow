...
func TestListFallsBackToEmbeddedCatalogWhenDefaultUnavailable(t *testing.T) {
	...
	if state.Source != "embedded" || len(entries) != 37 {
...
func TestExplicitUnsignedLocalRepositoryIsSelfContained(t *testing.T) {
	...
	if len(entries) != 37 || state.ActiveVersion != 1 || state.KeyID != "" || state.Source != "local:"+filepath.Join(repository, CatalogFilename) {
