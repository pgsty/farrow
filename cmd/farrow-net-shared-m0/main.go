// farrow-net-shared-m0 installs the pinned Darwin socket_vmnet shared-mode
// plan for native M0 diagnostics. It is intentionally not part of release
// packaging; the public Farrow CLI continues to reject shared mode.
package main
...
func main() {
	flags := flag.NewFlagSet("farrow-net-shared-m0", flag.ExitOnError)
	...
	if flags.NArg() != 0 || *archive == "" || *interfaceID == "" {
		fmt.Fprintln(os.Stderr, "usage: farrow-net-shared-m0 --archive <absolute-tar.gz> --interface-id <uuid> [--yes] [--json]")
		os.Exit(2)
	}
	if runtime.GOOS != "darwin" {
		fmt.Fprintln(os.Stderr, "farrow-net-shared-m0 requires native Darwin")
		os.Exit(3)
	}
