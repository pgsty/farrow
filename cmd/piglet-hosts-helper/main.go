// piglet-hosts-helper is the minimal root-owned publisher for a reviewed,
...
func run(args []string) error {
	flags := flag.NewFlagSet("piglet-hosts-helper", flag.ContinueOnError)
	...
	if flags.NArg() != 0 {
		return fmt.Errorf("piglet-hosts-helper does not accept positional arguments")
	}
	...
	if os.Geteuid() != 0 {
		return fmt.Errorf("piglet-hosts-helper must run as root")
	}
