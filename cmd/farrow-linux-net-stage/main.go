// farrow-linux-net-stage performs read-only host discovery and writes an
...
func main() {
	flags := flag.NewFlagSet("farrow-linux-net-stage", flag.ExitOnError)
	...
	if flags.NArg() != 0 || *staging == "" {
		fmt.Fprintln(os.Stderr, "usage: farrow-linux-net-stage --staging <absolute-empty-dir> [--cidr RFC1918/24]")
		os.Exit(2)
	}
	if runtime.GOOS != "linux" {
		fmt.Fprintln(os.Stderr, "farrow-linux-net-stage requires native Linux")
		os.Exit(3)
	}
