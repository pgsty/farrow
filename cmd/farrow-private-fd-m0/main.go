// farrow-private-fd-m0 exercises the product private controller while forcing
...
func main() {
	flags := flag.NewFlagSet("farrow-private-fd-m0", flag.ExitOnError)
	configPath := flags.String("f", "", "absolute private Farrow YAML")
	...
	if flags.NArg() != 0 || !filepath.IsAbs(*configPath) || !filepath.IsAbs(*workdir) {
		fmt.Fprintln(os.Stderr, "usage: farrow-private-fd-m0 -f <absolute-private-yaml> --work-dir <absolute-directory>")
		os.Exit(2)
	}
...
func run(configPath, workdir string, timeout time.Duration, record *evidence) (returnErr error) {
	...
	manager := privatevm.Manager{CWD: workdir, FarrowVersion: version.Version, ForceDarwinFD: true, ReadyTimeout: 3 * time.Minute}
	...
	for index, definition := range projectState.Resolved.Nodes {
		...
		args := vm.SSHArgsForUser(connection.User, connection.PrivateKey, connection.KnownHosts, connection.Port, "bash", "-lc", "ip -4 route get "+peer+"; /usr/local/libexec/farrow-network-check")
