...
and registered local image entries for the native host architecture.`,
  ...
  farrow --json image list`,
	...
	list.Flags().StringVar(&listOptions.Repository, "repo", "", repositoryFlagHelp)
...
--yes.`,
  ...
  farrow image prune --yes       # delete the displayed candidates`,
	...
	prune.Flags().BoolVar(&pruneOptions.DryRun, "dry-run", false, "show unreferenced images and stale staging files without deleting")
	prune.Flags().BoolVar(&pruneOptions.Apply, "yes", false, "delete the displayed unreferenced images and stale staging files")
...
prefixed alias and therefore also requires --boot and --source-user.`,
  ...
  farrow image import ./base.qcow2 --name local-dev --boot uefi --source-user dba`,
	...
	importCommand.Flags().StringVar(&importOptions.ExpectedSHA256, "sha256", "", "optional expected SHA-256")
	importCommand.Flags().StringVar(&importOptions.Name, "name", "", "optional immutable local- prefixed alias")
	importCommand.Flags().StringVar(&importOptions.Boot, "boot", "", "required with --name: bios or uefi")
	importCommand.Flags().StringVar(&importOptions.SourceUser, "source-user", "", "required with --name: source image login user")
