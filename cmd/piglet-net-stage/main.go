...
func main() {
	flags := flag.NewFlagSet("piglet-net-stage", flag.ExitOnError)
	...
	if flags.NArg() != 0 || *archive == "" || *staging == "" || *interfaceID == "" {
		fmt.Fprintln(os.Stderr, "usage: piglet-net-stage --archive <absolute-tar.gz> --staging <absolute-empty-dir> --interface-id <uuid> [--mode host|shared] [--cidr RFC1918/24]")
		os.Exit(2)
	}
	...
	plistSource := filepath.Join(*staging, "io.pgsty.piglet.vmnet.plist")
