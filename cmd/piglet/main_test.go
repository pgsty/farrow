...
func TestVersion(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "piglet dev") {
...
func TestQuickSSHAndStatusUseResolvedUser(t *testing.T) {
	...
	if !strings.Contains(human.String(), "ssh: operator@127.0.0.1:2222") || strings.Contains(human.String(), "dba@") {
	...
	status.SSHUser = ""
	printQuickStatus(&human, status)
	if !strings.Contains(human.String(), "ssh: dba@127.0.0.1:2222") {
...
func TestRollbackFlagIsPrivateUpOnly(t *testing.T) {
	var stdout, stderr bytes.Buffer
	if code := runQuickCommand("plan", []string{"--rollback"}, &stdout, &stderr); code != exitUsage || !strings.Contains(stderr.String(), "only valid with piglet up") {
...
func TestPlanMapsDataRootMigrationToConflict(t *testing.T) {
	...
	if err := (state.Store{Project: projectValue}).WriteProject(state.ProjectState{Schema: state.ProjectSchema, PigletVersion: "test", ProjectID: projectValue.Marker.ProjectID, SpecHash: hash, Resolved: persisted, UpdatedAt: time.Now().UTC()}); err != nil {
		t.Fatal(err)
	}
	configPath := filepath.Join(work, "piglet.yaml")
...
func TestValidateRejectsWorkspaceAsConfiguredDataRoot(t *testing.T) {
	...
	t.Setenv("PIGLET_DATA_HOME", "")
	var stdout, stderr bytes.Buffer
	if code := runValidate([]string{"-f", configPath}, &stdout, &stderr); code != exitUsage || !strings.Contains(stderr.String(), "unsafe broad Piglet data root") {
		t.Fatalf("code=%d stdout=%q stderr=%q", code, stdout.String(), stderr.String())
	}
}
...
func TestImageListJSON(t *testing.T) {
	t.Setenv("PIGLET_DATA_HOME", t.TempDir())
	var stdout, stderr bytes.Buffer
...
func TestInitEmbeddedProfilesAndOverrides(t *testing.T) {
	...
	if err != nil || file.Name != "meta" || len(file.Nodes) != 1 || !strings.Contains(stdout.String(), "# Piglet-owned profile: meta") {
...
func TestCompletions(t *testing.T) {
	...
	for _, shell := range []string{"bash", "zsh", "fish"} {
		...
		for _, want := range []string{"piglet", "image", "debug", "hosts", "project", "upgrade-state", "full", "simu"} {
			if !strings.Contains(stdout.String(), want) {
...
func TestPrivateProjectDestroyNeverFallsThroughToQuick(t *testing.T) {
	...
	if err := (state.Store{Project: projectValue}).WriteProject(state.ProjectState{Schema: state.ProjectSchema, PigletVersion: "test", ProjectID: projectValue.Marker.ProjectID, SpecHash: hash, Resolved: resolved, UpdatedAt: time.Now().UTC()}); err != nil {
		t.Fatal(err)
	}
	previous, err := os.Getwd()
	if err != nil {
